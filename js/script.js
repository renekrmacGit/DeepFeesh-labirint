...
// belezimo katere tipke drzimo pritisnjene
let keys = { ArrowUp: false, ArrowDown: false, ArrowLeft: false, ArrowRight: false, w: false, a: false, s: false, d: false };

// ko pritisnes tipko se to zapomne in starta stoperico, ce se ni
window.addEventListener('keydown', function(e) {
    if (keys.hasOwnProperty(e.key)) {
        keys[e.key] = true;
        e.preventDefault(); // da se ne premika cela stran
        if (startTime === 0 && isPlaying) startTime = Date.now();
    }
});
// ko spustis tipko, zabelezi da je spustena
...
        else if (keys.ArrowRight || keys.d) dx = 1;
        ...
        // preveri ce ni tam stene, da te spusti naprej
        if (dx !== 0 || dy !== 0) {
            let nx = player.x + dx, ny = player.y + dy;
            if (grid[ny][nx] === 0) {
                player.x = nx; player.y = ny;
                
                // belezimo sled, max 50 polj dolga
